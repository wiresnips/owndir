...
import { TransactionRow, addOrdinal } from './transactionTable'
...
function normalizeRent (transactions, days) {
  ...
  const dailyRent = toNearestCent( rent.amount * 12 / 365 );
...
function WeeklySpend ({transactions}) {
  ...
  return <table className={classes.join(' ')}>
    ...
    {spent.map((amount, d) => {
      amount = toNearestCent(amount);
      const wkEnd = latest.minus({days: d*7});
      const wkStart = latest.minus({days: ((d+1)*7)-1});
      const sameMonth = wkStart.month === wkEnd.month;

      return <tr key={d}>
        <td style={{"--size": Math.min(amount, chartMax) / chartMax}}>
          <span className={`data top-left`}>
            {wkStart.toFormat('LLL d')} - {wkEnd.toFormat(sameMonth ? 'd' : 'LLL d')}<br/>
            <span className='money'>{amount.toFixed(2)}</span>
          </span>
        </td>
      </tr>
