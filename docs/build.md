...
build/ 
  <hash>/      # staging area for the build of the whole owndir directory tree
    module/      # package exposing the Assembled `.owndir` code
    client/      # copied here from /.owndir/build/client
      bundler/     # bundles client/module into a single `dist.js`
      module/      # provides a client-side entrypoint (ie, attaches a react app into the DOM, connects FsNode interface to the server)
      dist.js      # the final client bundle, generated by 
    server/      # copied here from /.owndir/build/server
      bundler/     # bundles server/module into a single `dist.js`
      module/      # provides a server-side entrypoint (ie, exposes FsNode interface, serves custom routes, etc)
```

Roughly speaking, that process is this:

1- the code from each `/.owndir/` folder in the target directory-tree will be **assembled** into a single js package, which is placed in `build/<hash>/module/`
...
1- establish a "module build directory", which will contain all generated artifacts, `build/<hash>/module/`
2- each directory in the target tree which has a `.owndir` will be treated as a package, and copied into `build/<hash>/module/modules/<unique_module_id>`
...
/* 
  ...
  wherein we: 
    - establish an index of the necessary modules
...
- `<client-or-server>/module`
    This is a package that provides the overall entrypoint for the platform. 
      client: 
        - require `../../module` as a dependency, to access the **Assembled** package.
          - ie, by putting `{"dependencies": {"owndir.assembled": "../../module"}}` into `package.json`
        - set up a react-router that walks the owndir module hierarchy based on the url path, and renders from wherever that lands
          - NOTICE: this is where the method that a module must provide to show a custom view comes from.
            In my work so far, I've decided that a module can implement `frame`, which 
          - NOTICE: yes, react. This decision IS made in the client bundler, but not in the `/module` portion. It's implicit in the default way that `esbuild` handles `.jsx` files.
      ...
      note: this is being actively refactored. I expect the server module to take ownership of the server-side routing. There is a tight coupling between the server-side routing and the client, and this has lopsided consequences for the build. The client bundler module is doing a LOT of work to meet the server, but the server bundler module is just a passthrough precisely BECAUSE that functionality was baked into the core. Moving it out into the default server module instead makes it more overridable, and makes the relationship between the server and client builds more explicable.
